   ...
   // public GameObject spike;
    ...
    void Update()
    {
        if (Time.timeScale != 0 && isnotDied)
        {
            ...
            this.transform.position = new Vector3(this.transform.position.x, this.transform.position.y - (5F * Time.deltaTime), this.transform.position.z); //On gere le déplacement de haut en bas
            if (screenPos.y < 0.0F)
            {
                Destroy(gameObject);
            }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.tag == "Bullet") //si on est touché par une balle, on perd de la vie en fonction du type de balle
        {
        ...
        if (currentHealth <= 0) //si on meurt, on émet le son et on lance la coroutine de mort
        {
            ...
            if (other.GetComponent<BulletFragment>()) //si on est touché par une balle a fragmentation, on émet d'autres balles
            {
                for (int i = 0; i < 8; i++)
